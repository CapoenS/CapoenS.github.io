...
A prehistoric-themed digital card game (Hearthstone-style) that runs entirely in your browser. Plain HTML/CSS/JS — no frameworks, no build step, no server required.
...
- Your **amber** is shown as a cluster of glowing faceted gems at the bottom right of the battlefield (enemy's at the top right).
- **Drag a glowing card** from your hand onto the battlefield to play it. Creatures drop onto your board, biomes onto the biome banner, untargeted events anywhere on the field, and targeted events directly onto a highlighted creature. Invalid drops snap back to your hand.
- **Click a glowing creature** on your board, then click a highlighted target (creature or enemy leader) to attack.
- Card shapes tell you the type at a glance: creatures are era-crowned slabs with a tribal medallion, events are angular arcane shards with an animated energy edge, and biomes are arched land tablets. Keyword creatures wear distinctive frames and a badge on the top edge (stone double wall = Taunt, pulsing green = Venom, dashed silver = Stealth, crackling gold = Swift, breathing green glow = Regenerate, flickering embers = Frenzy, drifting cyan gusts = Evasive, golden shimmer = 7+ cost apex predators).
- **End Turn** passes to the AI. **New Game** reshuffles fresh random decks.
- 🔊 toggles sound.
...
- **Creatures** — Cost, ⚔ ATK, 🛡 Armor (flat damage reduction per hit), ❤ HP, a Tribe (shown as a colored tint on the card body and art medallion — borders are reserved for keywords and Legendary status), an Era (shown in the card subtitle), and possibly keywords:
  - 🛡️ **Taunt** — enemies must attack this first (Stealth attackers may ignore it)
  - ☠️ **Venom** — any nonzero combat damage it deals or receives destroys the other creature outright
  - 🌫️ **Stealth** — can't be targeted by attacks or targeted events until it attacks
  - ⚡ **Swift** — no summoning sickness
...
- **Events** — one-shot spells (damage, draw, buff, freeze…). Event damage **bypasses armor**.
...
Each creature's card is tinted by its Tribe: Aquatic ocean blue, Arthropod chitinous yellow-green, Reptile scale green, Bird golden plumage, and Mammal russet fur. The tribe also matters mechanically — biome cards buff specific tribes. A creature's Era appears as text in the card subtitle and tooltip. Events and biomes have no tribe and use their own purple-shard and land-tablet looks.

### Combat math

Combat is a **simultaneous exchange**: the attacker deals `ATK − target Armor` and the defender strikes back for `ATK − attacker Armor` (both floored at 0) — even if one of them dies in the process. Evasive attackers take no counterattack damage. Armor does **not** reduce event or biome damage.
...
│   ├── game.js         game state, turn flow, playing cards, events, boot code
│   ├── combat.js       attack legality, damage math, keywords, deaths
│   ├── ai.js           heuristic AI opponent
│   ├── ui.js           DOM rendering + click interactions
